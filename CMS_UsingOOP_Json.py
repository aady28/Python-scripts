...
class Customer:
    ...
    def cusIntoDict(cus):
        return cus.__dict__
    @staticmethod
    def saveCustomerinFileUsingJson():
        fs=open("cusmgtJson.txt","w")
        json.dump(Customer.listCus,fs,default=Customer.cusIntoDict)
    ...
    def dictIntoCusAuto(dictCus):
        ...
        return cus
    @staticmethod
    def loadCustomerfromFileUsingJson():
        fs = open("cusmgtJson.txt", "r")
        Customer.listCus=json.load(fs,object_hook=Customer.dictIntoCusAuto)
    def __init__(self):
        self.id=0
        self.name=""
        self.address=""
...
while(True):
    ...
    if(ch=="1"):
       ...
       Customer.saveCustomerinFileUsingJson()
       print("File save sucessfully")
    elif (ch == "8"):
        Customer.loadCustomerfromFileUsingJson()
